...
%%
myVars = {'train_x','train_y','train_nnx','train_nny','val_x','val_y','test_x','test_y'};
load('crblp_dbm_class_269.mat',myVars{:});
X = [train_x;train_nnx;val_x];
X_labels = [train_y;train_nny;val_y];
X_test=test_x;
X_test_labels=test_y;

%% for bugs in laptop
% matObj = matfile('crblp_dbm_class_269.mat');
% X = matObj.test_x(1:100, :);
% X_labels = matObj.test_y(1:100, :);
% X_test=matObj.test_x(101:150, :);
% X_test_labels=matObj.test_y(101:150, :);
...
if pretrain
    ...
    for l = 1:n_layers
        ...
        R.learning.minibatch_sz = 10;

        % max. 100 epochs
        R.iteration.n_epochs = 5;           %-----------------------------
...
% add by sankar at 25/03/2014
new_labels = zeros(size(X_test_labels,1), 1);
if size(X_test_labels,2) ~= 1    
    for ti = 1:size(X_test_labels,1)
        new_labels(ti) = find(X_test_labels(ti,:)==1); %class to value convert
    end
end
%---------------------------------------------------------
n_correct = sum(new_labels == pred);
